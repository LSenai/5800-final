...
def preprocess_wav(filepath):
    with wave.open(filepath, 'rb') as wavefile:
        ...
        # Scale the audio data to between 0 and 1
        max_val = max(abs(max(padded_audio_data)), abs(min(padded_audio_data)))
        scaled_audio_data = [x / max_val for x in padded_audio_data]
